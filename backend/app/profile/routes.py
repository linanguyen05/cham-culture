...
async def get_profile(
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            ...
            is_private = not bool(user_row.get("public_status", True) if user_row.get("public_status") is not None else True)
            can_view_posts = is_current_user or (not is_private) or is_following
    ...
    profile_user: dict[str, Any] = {
        ...
        "public_status": bool(user_row.get("public_status", True) if user_row.get("public_status") is not None else True),
    }
    if is_current_user:
